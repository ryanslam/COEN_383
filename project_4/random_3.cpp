...
{
    //If there is only one item in the list, we are done.
...
processNode* newRandomFunction()
{
	int randomProcessNum = rand() % 150;
	processNode* curProcess = processHead;
	for(int i = 0; i < randomProcessNum; i++)
	{
...
{
	processNode* curProcess = newRandomFunction();
	
	while(curProcess->startTime == -1)
	{
		processNode* curProcess = newRandomFunction();
	}
    ...
    {
		processNode* curProcess = newRandomFunction();
		head = curProcess->pageHead;
	}
	//If we only have one page, then we have to use that one page.
	if( head->next == NULL )
        {
			cout << "ONE NODE" << endl;
        ...
        {
			int count = 0;
			for(pageNode* head_2 = curProcess->pageHead; head_2 != NULL; head_2 = head_2->next)
			{
				count++;
			}
			int randomPage = rand() % count;
			...
			pageNode* second_last = curProcess->pageHead;
			
			//Delete and replace head
			if(randomPage == 0)
			{
			...
			{
				cout << "MULTIPLE NODES" << endl;
				while(second_last->next != head)
				{
					second_last = second_last->next;
				}
				freePageHead = head;
				second_last->next = head->next;
				cout << "Multiple Pages Owned: ";
                cout << "\tPage "<< freePageHead->frameId;
                cout << "\tEvicted from Process " << freePageHead->usingProcessId << endl; 
                remainingFreePages++;
