...
void findAndRemovePage() {
    int biggest_ref = INT_MIN;
    int mfu;
    // cout << "ARRAY" << endl;
    for(int i = 0; i < NUM_PAGES; i++) {
        // cout << "page " << i << " count: " << pageCounter[i] << endl; 
        if(pageCounter[i] > biggest_ref) {
            biggest_ref = pageCounter[i];
            mfu = i;
        }
    }

    cout << "Page " << mfu << " Evicted from Process " << memMap[mfu] << endl; 
    // cout << "PAGE TO REMOVE IS " << mfu << endl;
    // cout << "IN PROCESSS " << memMap[mfu] << endl;
    ...
    for(processNode* curProcess = processHead; curProcess != NULL; curProcess = curProcess->next) {
        ...
        if(curProcess->processId != memMap[mfu]) {
            continue;
        }
        ...
        // }
        // cout << "FINISHED ITERATING THROUGH LINKED LIST" << endl;
        // check if linked list is of size 1
        if(curProcess->pageHead->next == NULL) {
            // check if the node has frameId with value mfu
            if(curProcess->pageHead->frameId == mfu) {
                curProcess->pageHead = NULL;
                freePageHead = new pageNode;
                freePageHead->frameId = mfu;
            ...
            // check if head of linked list has frameId with value mfu
            if(curProcess->pageHead->frameId == mfu) {
                ...
                // iterate through the linked list and see if there is a node with frameId = mfu
                for(pageNode* curPage = curProcess->pageHead; curPage != NULL; curPage = curPage->next) {
                    if(curPage->next->frameId == mfu) {
